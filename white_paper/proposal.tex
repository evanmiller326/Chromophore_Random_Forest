...
Reducing computational time is a critical step for facilitating large-scale parameter sweeps harnessing high-performance computing clusters to identify the best candidates for efficient organic electronic devices.
As an example, open data repositories such as the Harvard Clean Energy Project database contain millions of candidate molecules - a currently untenably large dataset for investigating charge transport properties.
Our work here will allow researchers to investigate many more of these systems at many more statepoints in the same amount of time, helping us to more strongly connect morphology and performance, with the aim of constructing a robust set of design rules for the most efficient devices.
...
Ideally, we already have sufficient data to construct a sufficiently accurate machine learning model, and so we envisage not having to perform significantly more quantum chemical calculations.
The worst-case scenario according to our preliminary results is that we require the generation of a new model for each new chemistry.
...
All preliminary data has been generated using the open source MorphCT\cite{Jones2017}, HOOMD-Blue\cite{Anderson08}, and ORCA\cite{Neese2012b} software suites, and are currently hosted online by the Albertson's Library at Boise State University \textcolor{red}{CITE}.
The morphologies were generated using local computing hybrid CPU/GPU infrastructure at Boise State University, funded by \textcolor{red}{CITE FUNDING}.
These resources will be also be available for subsequent data generation for this proposal.
In addition, we have access to 5,000 SUs through XSEDE on the high-performance GPU cluster XSTREAM, with the intent to apply for an increase to that allocation in the future.
